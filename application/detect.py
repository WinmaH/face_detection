...
if match >= mismatch:
    ...
    window = Tk()
    window.withdraw()
    tkMessageBox.showerror('Error', 'Cannot Register to the system??? User is already registered ?')
else:
    ...
    def show_name():
        ...
        while (True):
            ...
            for (xf, yf, wf, hf) in faces:
                # taking the grey view of the frame

                interested_region = gray[yf:yf + hf, xf:xf + wf]
                interested_region_color = frame[yf:yf + hf, xf:xf + wf]
                cv2.rectangle(interested_region_color, (xf, yf), (xf+ wf, yf + hf), (255, 0, 0), 2)

                if photoframe==10:
                    interested_region = gray[yf:yf + hf, xf:xf + wf]
                    cv2.imwrite(os.path.join(file_path,'pic1.png'),interested_region)
                elif photoframe==20:
                    interested_region = gray[yf:yf + hf, xf:xf + wf]

                    cv2.imwrite(os.path.join(file_path, 'pic2.png'), interested_region)
                elif photoframe==30:
                    interested_region = gray[yf:yf + hf, xf:xf + wf]

                    cv2.imwrite(os.path.join(file_path, 'pic3.png'), interested_region)
                elif photoframe==40:
                    interested_region = gray[yf:yf + hf, xf:xf + wf]

                    cv2.imwrite(os.path.join(file_path, 'pic4.png'), interested_region)
                elif photoframe==50:
                    interested_region = gray[yf:yf + hf, xf:xf + wf]

                    cv2.imwrite(os.path.join(file_path, 'pic5.png'), interested_region)
                elif photoframe==60:
                    interested_region = gray[yf:yf + hf, xf:xf + wf]

                    cv2.imwrite(os.path.join(file_path, 'pic6.png'), interested_region)
                elif photoframe==70:
                    interested_region = gray[yf:yf + hf, xf:xf + wf]

                    cv2.imwrite(os.path.join(file_path, 'pic7.png'), interested_region)
